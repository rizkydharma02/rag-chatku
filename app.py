...
st.set_page_config(
    page_title="RAG AI Groq",
    page_icon=":blue_heart:",
)
st.title("Chatku AI")
st.markdown("##### Chat PDF dan URL berbasis RAG Llama3 dengan Groq")
st.markdown("##### dibuat dengan :blue_heart: oleh Rizky Dharma A.R")
...
def main() -> None:
    # Get LLM model
    llm_model = st.sidebar.selectbox("Pilih LLMs", options=["llama3-70b-8192", "llama3-8b-8192", "mixtral-8x7b-32768"])
    ...
    embeddings_model = st.sidebar.selectbox(
        "Pilih Penyematan(Embeddings)",
        options=["nomic-embed-text", "text-embedding-3-small"],
        help="Saat Anda mengubah model penyematan, dokumen perlu ditambahkan lagi.",
    )
    # Set assistant_type in session state
    ...
    try:
        ...
        st.warning("Tidak dapat membuat asisten, apakah database berjalan?")
        return

    # Load existing messages
    assistant_chat_history = rag_assistant.memory.get_chat_history()
    if len(assistant_chat_history) > 0:
        logger.debug("Memuat riwayat obrolan")
        st.session_state["messages"] = assistant_chat_history
    else:
        logger.debug("Tidak ada riwayat obrolan yang ditemukan")
        st.session_state["messages"] = [{"role": "assistant", "content": "Unggah dokumen dan ajukan pertanyaan..."}]
    ...
    if rag_assistant.knowledge_base:
        ...
        input_url = st.sidebar.text_input(
            "Masukkan URL Website", type="default", key=st.session_state["url_scrape_key"]
        )
        add_url_button = st.sidebar.button("Tambahkan URL")
        if add_url_button:
            if input_url is not None:
                alert = st.sidebar.info("Memuat URLs...", icon="ℹ️")
                if f"{input_url}_scraped" not in st.session_state:
                    ...
                    if web_documents:
                        ...
                        st.sidebar.error("Tidak dapat membaca situs web")
        ...
        if uploaded_file is not None:
            alert = st.sidebar.info("Proses PDF...", icon="🧠")
    ...
    if rag_assistant.knowledge_base and rag_assistant.knowledge_base.vector_db:
        if st.sidebar.button("Hapus Data"):
            ...
            st.sidebar.success("Data Telah Terhapus!")
    ...
    if st.sidebar.button("Jalankan Baru"):
        restart_assistant()

    if "embeddings_model_updated" in st.session_state:
        st.sidebar.info("Harap tambahkan lagi dokumen karena model penyematan telah berubah.")
